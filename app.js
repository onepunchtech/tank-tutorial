...
function fireShot(player) {
    if (player.keys.fire === true) {
        let radians = player.tank.direction * (Math.PI / 180);
        let now = new Date().getTime().valueOf();
        const lastFired = player.lastFired;
        if (lastFired + (fireRate * 1000) < now) {
            const roundX =  player.tank.pos.x + (shotVelocity) * Math.cos(radians);
            const roundY =  player.tank.pos.y + (shotVelocity) * Math.sin(radians);
            const newRound = {
                direction: player.tank.direction,
                pos: { x: roundX, y: roundY},
                velocity: shotVelocity,
            };
            player.shots.push(newRound);
